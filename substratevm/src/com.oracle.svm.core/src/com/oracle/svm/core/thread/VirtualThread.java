...
final class VirtualThread extends Thread {
    ...
    private void mount() {
        ...
        if (JavaThreads.isInterrupted(this)) {
            JavaThreads.platformGetAndUpdateInterrupt(carrier, true);
        } else if (JavaThreads.isInterrupted(carrier)) {
            synchronized (interruptLock()) {
                if (!JavaThreads.isInterrupted(this)) {
                    JavaThreads.platformGetAndUpdateInterrupt(carrier, false);
                }
            }
        }
    ...
    public boolean getAndClearInterrupt() {
        assert Thread.currentThread() == this;
        synchronized (interruptLock()) {
            return JavaThreads.platformGetAndUpdateInterrupt(carrierThread, false);
        }
    }
    ...
    private static ScheduledExecutorService createDelayedTaskScheduler() {
        int poolSize = 1;
        ScheduledThreadPoolExecutor dts = (ScheduledThreadPoolExecutor) Executors.newScheduledThreadPool(poolSize, task -> InnocuousThread.newThread("VirtualThread-unparker", task));
